...
#define MQTT_NAME "shaun_7089"
#define MQTT_PASS ""

int SERVO_PIN = 2;    // The pin which the servo is attached to
...
void loop()
{
   ...
   Serial.println(hh);
   lcd.print("Time:");
   if(mm==minutes and ss<7){
    if(hh==morningHour+12){
      open_door();
        delay(3000);
        close_door();
    }
    else if (hh==afternoonHour+12){
 open_door();
        delay(3000);
        close_door();
    }
    else if (hh==eveningHour+12){
 open_door();
        delay(3000);
        close_door();
    }
